...
def _():
    ...
    > **Note**: have a look at our [pyMultiwfn repository](https://github.com/szczypinski-group/pyMultiwfn) for a real-life example.

    On top of the `pyproject.toml` generated by `uv`, you can add some useful (potentially) settings for programming tools:
    ...
    lint.ignore = [
        "D102",  # Missing docstring in public method (interrogate)
        "D103",  # Missing docstring in public function (interrogate)
        "D105",
        "TD003",  # TODO GH issue links are not always possible
    ]
    ...
    To ensure that code is uniform and "up to standard", we can use tools like pre-commit. This stops the developer from sending inconsistent code to GitHub by introducing a number of tests that must pass. Example configuration could look like this:
